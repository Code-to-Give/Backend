...
from db.models import RequirementModel, AgencyModel
from schemas.Requirement import Requirement, RequirementCreated
...
async def create_requirement(requirement: RequirementCreated, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    result = await db.execute(
        ...
        .filter(RequirementModel.agency_id == agency.id, RequirementModel.food_type == requirement.food_type)
    )
    db_requirement = result.scalar_one_or_none()
    if db_requirement is None:
        raise HTTPException(
            status_code=404, detail="Requirement not found after upsert")
